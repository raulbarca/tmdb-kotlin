...
import com.barca.tmdb.core.interactor.UseCase
...
class GetMoviesTest : UnitTest() {
    ...
    private lateinit var getUpcomingMovies: GetUpcomingMovies

    @Mock private lateinit var moviesRepository: MoviesRepository

    @Before fun setUp() {
        getUpcomingMovies = GetUpcomingMovies(moviesRepository)
        given { moviesRepository.upcomingMovies() }.willReturn(Right(listOf(Movie.empty())))
    }

    @Test fun `should get data from repository`() {
        runBlocking { getUpcomingMovies.run(/*UseCase.None()*/1) }

        verify(moviesRepository).upcomingMovies()
